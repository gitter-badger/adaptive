...
class Triangulation:
    ...
    def vertex_in_simplex(self, simplex, vertex, eps=1e-8):
        ...
        alpha = np.linalg.solve(vectors.T, vertex - x0)
    ...
    def extend_hull(self, new_vertex, eps=1e-8):
    ...
    def add_vertex_inside_simplex(self, vertex, simplex):
        if len(self.vertex_in_simplex(simplex, vertex)) != self.dim + 1:
    ...
    def add_vertex_on_face(self, vertex, face):
        pt_index = len(self.vertices)
        self.vertices.append(vertex)

        simplices = self.containing(face)
        if (set(self.vertex_in_simplex(vertex, next(iter(simplices))))
            != set(face)):

            raise ValueError("Vertex does not lie on the face.")

        if all(pt in self.hull for pt in face):
            self.hull.append(pt_index)
